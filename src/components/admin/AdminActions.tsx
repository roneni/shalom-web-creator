...
export default function AdminActions({ onLogout }: AdminActionsProps) {
  ...
  const anyBusy = isFetching || isSearching || isTrending || isProcessing || isFetchingLikes;
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <Button onClick={handleFetchTwitterLikes} disabled={anyBusy} size="sm" variant="outline"
        className="border-pink-500/50 text-pink-600 hover:bg-pink-50 dark:hover:bg-pink-950/20">
        {isFetchingLikes ? <Loader2 className="h-4 w-4 animate-spin ml-2" /> : <Heart className="h-4 w-4 ml-2" />}
        לייקים
      </Button>
      <Button onClick={handleTrendingSearch} disabled={anyBusy} size="sm" variant="outline"
        className="border-destructive/50 text-destructive hover:bg-destructive/10">
        {isTrending ? <Loader2 className="h-4 w-4 animate-spin ml-2" /> : <TrendingUp className="h-4 w-4 ml-2" />}
        טרנדים
      </Button>
      <Button onClick={handleSearchContent} disabled={anyBusy} size="sm" variant="outline">
        {isSearching ? <Loader2 className="h-4 w-4 animate-spin ml-2" /> : <Search className="h-4 w-4 ml-2" />}
        חפש חדשות
      </Button>
      <Button onClick={handleProcessOnly} disabled={anyBusy} size="sm" variant="outline"
        className="border-primary/50 text-primary hover:bg-primary/10">
        {isProcessing ? <Loader2 className="h-4 w-4 animate-spin ml-2" /> : <Cpu className="h-4 w-4 ml-2" />}
        עבד הצעות
      </Button>
      <Button onClick={handleFetchAndProcess} disabled={anyBusy} size="sm">
        {isFetching ? <Loader2 className="h-4 w-4 animate-spin ml-2" /> : <RefreshCw className="h-4 w-4 ml-2" />}
        שלוף ועבד
      </Button>
