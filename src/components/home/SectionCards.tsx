...
import { sections, getLatestPostBySection } from "@/data/mockData";

const SectionCards = () => {
  return (
    <section className="container mx-auto px-4 py-20">
      <h2 className="text-2xl md:text-3xl font-bold text-center mb-12">
        ארבעה מדורים, <span className="gradient-text">אפס רעש</span>
      </h2>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {sections.map((section) => {
          const latestPost = getLatestPostBySection(section.id);
          return (
            <div
              key={section.id}
              className="group rounded-xl bg-card border border-border hover:border-primary/30 transition-all duration-300 hover:glow-sm"
            >
              <div className="p-6">
